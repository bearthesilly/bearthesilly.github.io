...
9. 通过修改 Dijkstra 算法，在所有最短路径中选择边数最少的路径：给所有的边长都加上$\epsilon$，其中$\epsilon < \delta/|E|$，然后运行Dijkstra算法。这是因为在两条路径拥有相同权重的同时，边长数量较少的路径将有较小的$\epsilon$综合。
