...
因此pop push的时间复杂度是O(n)，而在二叉树高度不平衡的时候，会达到worst case。
...
> More specifically, if we are inserting an object less than the root (at the front), then the run time will be $O(ln(n))$; if we insert at the back (greater than any object) then the run time will be $O(n)$.

关于push操作，如果我们随意地插入呢？那么平均时间复杂度是多少呢？
...
二叉堆的操作的时间复杂度（其中build method是Floyd'd Method）以及堆排序的时间和空间复杂度：
