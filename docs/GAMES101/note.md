...
最后介绍光场（Light Field / Lumigraph）。我们生活中看到现实三维世界，但是假如说眼前放一块屏幕，然后精确呈现你之前看到的2D画面，那么你可能不会发现异常，这就是虚拟现实的原理（我们只关心“幕布”，不关心真实的3D世界）。那么如何表示人看到的2D呢？我们可以使用全光函数（Plenoptic Function）来进行表示。以眼睛为原点，可以用球坐标系的输入来表示函数：$P(\theta,\phi) \rightarrow Intensity\ of\ light$。这个函数没有RGB值，因此可以进行改进：$P(\theta,\phi, \lambda)$。同时，我们可以引入时间维度，同时引入眼睛的三维坐标：$P(\theta,\phi, \lambda, t, V_{x}, V_{y}, V_{z})$。这是一个七个维度的函数。

三维中的光线认为是5D：起点三维，方向球坐标表示两维。那么什么是光场？见下图。我们用盒子框住物体，然后用一定的resolution划分盒子上的表面。对于物体表面的任何一个点来说，它和盒子上的一个小格子的中心点连线，就是方向，如果通过这些信息我就能知道intensity的话，这就是光场的概念。同时，由于物体表面可以理解为2D（texture），因此实际上这个函数（全光函数的一部分）的输入维度就从原来的5D降到4D：2D表示位置（2D coordinates inputted into mapping），2D表示方向。有了光场，就能知道任意角度看向物体各个点所应该接受到的光。

<img src="img/153.png" alt="image" style="zoom:33%;" />

同时，与其专门用两个角来表示方向，还可以使用两个点。如下图：定义两个相互平行的面，而且是Axis-Aligned，那么通过四个参数，即两个点各自的二维信息，就可以通过输入函数求出光强。

<img src="img/154.png" alt="image" style="zoom:33%;" />

利用光场，就能制作光场摄像机：原来的每一个像素对应的sensor都换成透镜，对于3D世界中的每一个方向过来的点，都能用光场函数知道强度，因此到达棱镜时的是irradiance，但是透过透镜又能按照方向分开储存radiance；相当于，如果分开储存的那些radiance都取平均，那么这就是一个原本的普通相机。这么做有什么awesome的地方吗？那就是虚拟变焦！通过计算某一焦距下各个像素应该选出的irradiance来填充各个像素，得到的照片就是我希望的焦距下的照片。当然分辨率会下降，因为原来呈现一个像素只需要胶片的一个单位，但是现在为了记录多个方向的radiance，需要额外消耗储存空间。

<img src="img/155.jpg" alt="image" style="zoom:33%;" />

## Color and Perception

一束光穿过三棱镜，能折射出不同的颜色，是因为不同波长的光折射率不一样；换而言之，光其实是由不同波长构成的，而对应到的就是光谱（The Visible Spectrum of Light）。同样，我们也可以用普功率密度来描述光在不同波长的分布（SPD）。SPD有线性性质（Linearity）。

但是最终我们看到的不是光谱，也不是SPD曲线：Color is a phenomenon of human perception; it is not a universal property of light。那么人究竟是如何感知到颜色的？那么就要问眼睛的结构了。视网膜是最终光线到达的地方，那么颜色的呈现是由于视网膜上的感光细胞：细胞有两种，一种是棒状，可以用来感知光照的强度，第二种是锥形细胞，它还分为三类，每一类对不同的波长的感受程度不一样。

<img src="img/156.png" alt="image" style="zoom:33%;" />

所以人眼究竟感受到的是什么？其实上述的三种细胞的感受曲线其实是意味着的是“敏感度”，那么通过三个权重曲线去和原来光线的波长进行积分操作，就能得到三个值：SML，如下图所示。最终，人眼看见的其实就是三种对应的值：SML，这是大脑收到的信息。

<img src="img/157.png" alt="image" style="zoom:33%;" />

接下来介绍一种现象：同色异谱（Metamerism）。不同波长可能有相同的颜色吗？答案当然是可能的。如下图：

<img src="img/158.png" alt="image" style="zoom:33%;" />

Color Reproduction / Matching。Additive Color is as the following figure:


<img src="img/159.png" alt="image" style="zoom:33%;" />

说明通过三种波长，我就能拟合出很多的颜色。当然有一些特殊的情况：如下图，左边的颜色右边怎么也拟合不出来，但是如果左边加上一种波长的一定强度，右边就能拟合出来了，这种情况大多是因为波长只能相加，不能在实际操作中实现减法。

<img src="img/160.png" alt="image" style="zoom:33%;" />

因此，人们尝试用RGB去拟合颜色。下图就展示了：不同波长下的光，应该如何用三种RGB波长光的各自参数去拟合(Standardized RGB, sRGB)：

<img src="img/161.jpg" alt="image" style="zoom:33%;" />

虽然sRGB看似完美，但是能够拟合的波长光的波长范围，也就是色域，是有限的。上图中红色负数的部分在现实操作中不可能实现，因为我们只能给光加上波长光，不能抽出波长光。因此还有其他的Color Matching，如CIE XYZ，它和sRGB的区别仅仅是匹配函数。色域，Gamut，is the set of chromaticties generated by a set of color primaries。还有些常见的颜色空间，如HSV（Hue-Saturation-Value），Lab（CIELAB Space），CMKY（Cyan, Magenta, Yellow, and Key）。

<img src="img/163.png" alt="image" style="zoom:33%;" />

<img src="img/162.png" alt="image" style="zoom:33%;" />
